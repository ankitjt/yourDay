firebase.auth().onAuthStateChanged( ( user ) =>
{
   if ( user )
   {
      ...
      let secondNav = `
         <nav
                ...
                <a href="./settings.html"
                   class="navLink flex ease-in-out duration-300 hover:bg-indigo-900 sm:hover:bg-transparent md:hover:bg-indigo-900 hover:text-white p-3 sm:mb-2 items-center uppercase justify-between sm:justify-center md:justify-between w-full group rounded-xl text-xs font-semibold tracking-widest"
                   title="Settings">
                  <span class="inline sm:hidden md:inline">${ user.displayName }</span>
                  <svg xmlns=" http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                       stroke="currentColor"
                       class="md:w-4 md:h-4 w-6 h-6 md:ml-3 sm:mr-0 ease-in-out duration-300 group-hover:text-white sm:group-hover:text-indigo-900 md:group-hover:text-white">
                    <path stroke-linecap="round" stroke-linejoin="round"
      ...
      // For smaller screens menu open option.
